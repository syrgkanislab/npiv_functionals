...
class _BaseAGMM:
    ...
    def _pretrain(self, Z, T, Y,
        ...
        self.optimizerD = OAdam(add_weight_decay(self.learner, learner_l2),
                                lr=learner_lr, betas=betas)
        self.optimizerG = OAdam(add_weight_decay(
            self.adversary, adversary_l2, skip_list=self.skip_list), lr=adversary_lr, betas=betas)
...
class _BaseSupLossAGMM(_BaseAGMM):
    ...
    def fit(self, Z, T, Y, Z_dev, T_dev, Y_dev, eval_freq=1,
            ...
            learner_tikhonov=0, n_earlystop_funcs=100, earlystop_save_every=1,
        ...
        # cannot run in both riesz and direct riesz modes simultaneously
        assert not (riesz and direct_riesz)
        ...
        # early_stopping
        n_earlystop_epochs = n_earlystop_funcs * earlystop_save_every
        f_of_z_dev_collection, m_of_f_of_z_dev_collection = self._earlystop_eval(Z, T, Y, Z_dev, T_dev, Y_dev,
                                                                                 device, n_earlystop_epochs, earlystop_save_every,
                                                                                 ols_weight, adversary_norm_reg, learner_tikhonov,
        ...
        for epoch in range(n_epochs):
            ...
            for it, (zb, xb, yb) in enumerate(self.train_dl):
                ...
                if (it % train_learner_every == 0):
                    ...
                    if riesz:
                        D_loss = torch.mean(
                            pred * test - moment_fn(xb, self.learner))
                    elif direct_riesz:
            ...
            if (epoch % eval_freq == 0) & (epoch > min_eval_epoch):
                ...
                if riesz:
                    ...
                    m_of_g_of_x_dev = moment_fn(T_dev, self.learner)
    ...
    def _earlystop_eval(self, Z_train, T_train, Y_train, Z_dev, T_dev, Y_dev, device=None,
                        n_epochs=60, save_every=1,
                ...
                with torch.no_grad():
                    ...
                    if direct_riesz:
                        m_of_f_of_z_dev_collection.append(
                            moment_fn(Z_dev, self.adversary))
